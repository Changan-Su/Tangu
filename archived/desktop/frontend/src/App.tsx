...
export function App(): React.JSX.Element {
  ...
  // 工作台右栏默认展开(可停靠 Surface 是新设计的看点);显隐持久化,与 Dockview 布局保持同步。
  const [rightOpen, setRightOpen] = useState(() => { try { return localStorage.getItem('forsion_tangu_right_open') !== '0' } catch { return true } })
  useEffect(() => { try { localStorage.setItem('forsion_tangu_right_open', rightOpen ? '1' : '0') } catch { /* private mode */ } }, [rightOpen])
  ...
  const [themeLang, setThemeLang] = useState(resolveInitialLang)
  const [themeSkin, setThemeSkin] = useState(resolveInitialSkin)
  ...
  const reduceEvent = useCallback((sessionId: string, runId: string, assistantRef: { current: string }, ev: AgentRunEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'tool_result':
        ...
        break
      case 'display_file': // agent 在对话区展示文件(display_file/生图/表情包):图片内联缩略图、点击放大
        patchMessage(sessionId, assistantId, (m) => ({
          ...m,
          displayFiles: [...(m.displayFiles || []), { name: pl.name, mime: pl.mime, path: pl.path, dataUrl: pl.dataUrl }],
        }))
        break
  ...
  const send = useCallback(async (text: string, attachments: Attachment[], workspaceFiles?: Attachment[], skillIds?: string[], mentions?: { priorityAgent?: string; mentionAgents?: string[] }): Promise<boolean> => {
    ...
    // 默认生图模型(全局设置):generate_image 缺省据此选模型(用户在工具里仍可指定 model 覆盖)。
    if (!agentConfig.imageModelId && cfgRef.current.imageModelId) agentConfig.imageModelId = cfgRef.current.imageModelId
  ...
  // 聊天模型选择器只列大语言模型;生图模型(image_gen)由 generate_image 工具按需取,不当聊天模型用。
  const visibleModels = !modelsResp?.models
    ? null
    : modelsResp.models.filter((m) => (m.modelType || 'llm') === 'llm' && (!isCloudSession || m.source === 'forsion'))
  ...
  // 工作台右栏 Surface 的共用 props(无会话时各 surface 出占位)。
  const rightPanelProps = activeId
    ? {
        cfg, sessionId: activeId, sessionConfig: execConfig, running,
        messages: activeMessages, chatScrollRef, onToast: toast,
        onOpenPreview: setFilePreview, subChats: subChatsBySession[activeId] || [],
      }
    : null
  const wbEmpty = <div className="panel-note" style={{ padding: '18px 12px' }}>{t('workbench.noSession')}</div>

  if (!cfgLoaded) return <div className="app" />

  // 侧栏作为工作台「navigator」停靠区的内容(可拖拽调宽 / 收起);宽度由 Dockview 控,见 workbench.css。
  const sidebarEl = (
    <Sidebar
  ...
  return (
    <div className="app">
      <Ribbon
        onOpenSettings={() => openSettings()}
        onToast={toast}
        onAuthChange={() => { setTimeout(() => void connect(cfgRef.current), 1500) }}
      />
      <main className="main">
        ...
        <ChatHeader
          ...
          onToggleMode={() => {
            ...
            applyTheme(themeLang, themeSkin, m, { customColor: themeSeed }) // 切换 + 持久化;custom 时带 seed
            setThemeMode(m)
          }}
        ...
        <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
          <Workbench
            dark={themeMode === 'dark'}
            soft={themeLang === 'soft'}
            navVisible={!sidebarCollapsed && !settingsOpen && !onboarding}
            rightVisible={rightOpen && !settingsOpen && !onboarding}
            t={t}
            slots={{
              navigator: sidebarEl,
              workspace: rightPanelProps ? <RightPanel {...rightPanelProps} view="workspace" /> : wbEmpty,
              toc: rightPanelProps ? <RightPanel {...rightPanelProps} view="toc" /> : wbEmpty,
              memory: rightPanelProps ? <RightPanel {...rightPanelProps} view="memory" /> : wbEmpty,
              subchats: rightPanelProps ? <RightPanel {...rightPanelProps} view="subchats" /> : wbEmpty,
              center: (
                settingsOpen ? (
            <SettingsModal
              open={settingsOpen}
            ...
            <OnboardingWizard
              ...
              themeMode={themeMode}
              themeSeed={themeSeed}
              onThemeChange={(lang, skin, mode) => { setThemeLang(lang); setThemeSkin(skin); setThemeMode(mode) }}
              onSeedChange={onSeedChange}
              onReconnect={() => {
