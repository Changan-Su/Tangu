...
export const AgentMemoryModal: React.FC<{
  ...
  const tabBtn = (id: Tab, label: string): React.ReactNode => (
    <button
      className="btn sm"
      onClick={() => setTab(id)}
      style={tab === id ? { background: 'var(--accent)', color: '#fff' } : { background: 'transparent' }}
    >{label}</button>
  )
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: 'var(--bg-card)', border: 'var(--border-width) solid var(--border)', borderRadius: 'var(--radius-lg, 12px)', padding: 18, width: 'min(680px, 92vw)', maxHeight: '86vh', overflow: 'auto', boxShadow: '0 12px 40px rgba(0,0,0,0.25)' }}
      >
        ...
        {tab === 'library' && (
          <div className="field">
            ...
            {libErr && <div className="hint" style={{ color: 'var(--danger, #dc2626)' }}>{libErr}</div>}
